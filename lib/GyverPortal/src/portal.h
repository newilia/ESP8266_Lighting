...
public:
    ...
    bool isFormSubmitted(const String& name) {
...
private:
    void checkList() 
    {
        ...
        for (int i = 0; i < list.idx; i++) 
        {
            ...
            if (!e.skip && !isFormSubmitted(e.formName))
            {
                continue;
            }
            switch (e.type) 
            {
            case T_CSTR:
                copyStr(e.name, (char*)e.val);
                break;
            case T_STRING:
                *(String*)e.val = getString(e.name);  
                break;
            case T_TIME:
                *(GPtime*)e.val = getTime(e.name);    
                break;
            case T_DATE:
                *(GPdate*)e.val = getDate(e.name);    
                break;
            case T_CHECK:
                *(bool*)e.val = getCheck(e.name);     
                break;
            case T_BYTE:
                *(int8_t*)e.val = (int8_t)getInt(e.name); 
                break;
            case T_INT:
                *(long*)e.val = getInt(e.name);       
                break;
            case T_FLOAT:
                *(float*)e.val = getFloat(e.name);    
                break;
            case T_COLOR:
                *(GPcolor*)e.val = getColor(e.name);  
                break;
            case T_COLOR_INT:
                *(uint32_t*)e.val = getColor(e.name).getHEX();  
                break;
            default:
                Serial.println(F("GyverPortal::checkList(): trying to parse undefined type of argument"));
                break;
            }
        }
    }
