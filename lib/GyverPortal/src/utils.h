...
struct GPcolor {
    ...
    inline void setHEX(uint32_t col) {
    ...
    inline void decode(const String& hex) {
...
struct GPdate {
    ...
    inline void decode(const String& str) {
...
// deprecated
inline String encodeDate(GPdate d) {
    return d.encode();
}
inline String encodeTime(GPtime t) {
    return t.encode();
}
inline String encodeColor(GPcolor c) {
    return c.encode();
}

// ===================== DATE-TIME UNIX =====================
inline uint32_t GPunix(uint16_t y, uint8_t m, uint8_t d, uint8_t h, uint8_t mn, uint8_t s, int8_t gmt = 0) {
    int8_t my = (m >= 3) ? 1 : 0;
    y += my - 1970;
    uint16_t dm = 0;
...
inline void GPaddInt(int16_t val, int16_t* arr, uint8_t am) {
    for (int i = 0; i < am - 1; i++) arr[i] = arr[i + 1];
    arr[am - 1] = val;
}
inline void GPaddUnix(uint32_t val, uint32_t* arr, uint8_t am) {
    for (int i = 0; i < am - 1; i++) arr[i] = arr[i + 1];
    arr[am - 1] = val;
}
inline void GPaddUnixS(int16_t val, uint32_t* arr, uint8_t am) {
    for (int i = 0; i < am - 1; i++) arr[i] = arr[i + 1];
    arr[am - 1] += val;
}
...
inline char* splitList(char* str) {
...
inline int8_t inList(const char* name, const char* list) {
...
inline int8_t inList(String& s, const char* list) {
    return inList(s.c_str(), list);
}

inline int8_t inList(const String& s, const char* list) {
    return inList(s.c_str(), list);
}